...
> Parte da skill **schematize-rust**. Continuação de `testes.md` (numeração **preservada**, §22.4+): padrão de script de teste, seeds/personas, integração com CI, pentest (§22.7–22.8) e o Makefile padrão (§23). O fluxo de Q.A. plan-first vive na skill dedicada **schematize-qa** (`/qa-plan` → `/qa-run`).
...
O fluxo de Q.A. plan-first da casa foi extraído para a skill dedicada **schematize-qa** (`/qa-plan` → `/qa-run`): planeja tudo, gera o MD de passo a passo no `<projeto>_archive/qa/`, pede aprovação ANTES de rodar, executa faseado/assistido ou de uma vez com watchdog, e trava nos gates. Nada de Q.A. roda sem plano aprovado.

No recorte **Rust**, o wrapper é `/rust-qa`: use `cargo test`/`cargo nextest` e os scripts de teste da skill (§22.4).
